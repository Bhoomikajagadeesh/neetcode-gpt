...
class CachedAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, kv_cache: Optional[KVCache] = None) -> Tuple[torch.Tensor, KVCache]:
        ...
        q=q.view(b,t,1,d).transpose(1,2)
        k=k.view(b,-1,1,d).transpose(1,2)
        v=v.view(b,-1,1,d).transpose(1,2)
        s=(q@k.transpose(-2,-1))/d**0.5
        w=F.softmax(s,dim=-1)
        o=(w@v).transpose(1,2).contiguous().view(b,t,d)
        return o.round(decimals=4),kv_cache
